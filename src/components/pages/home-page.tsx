      ...
      <div className="relative max-w-[1400px] mx-auto px-6 md:px-12 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center min-h-[calc(100vh-200px)]">
        ...
        <div className="flex flex-col gap-8 z-10">
          ...
          <p className="text-muted-foreground text-lg leading-relaxed max-w-md font-light">
            Navigating the intersection of timeless Vedic principles and contemporary strategic
            excellence — counsel for the elite visionary.
          </p>
          ...
          {orbitalNodes.map((n) => (
            <div key={n.label} className={`absolute ${n.pos} group cursor-pointer text-center`}>
              <div className="w-12 h-12 rounded-full border border-[var(--gold)]/30 bg-white/80 backdrop-blur flex items-center justify-center mb-2 transition-all duration-500 group-hover:scale-110 group-hover:border-[var(--gold)] group-hover:shadow-[0_0_24px_rgba(200,151,61,0.45)]">
                <Icon
                  name={n.icon}
                  className="text-[var(--gold)] group-hover:scale-110 transition-transform text-[20px]"
                />
              </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex flex-col md:flex-row items-center gap-8">
        ...
        <div className="flex-1 flex flex-wrap justify-around items-center gap-x-10 gap-y-4 opacity-80">
          {marks.map((m, i) => (
            <span
              key={m}
              className="font-display italic text-muted-foreground text-lg md:text-xl flex items-center gap-10"
            >
              {m}
              {i < marks.length - 1 && (
                <span className="hidden md:inline-block w-px h-4 bg-[var(--gold)]/30" />
              )}
            </span>
          ))}
        </div>
...
const services = [
  {
    icon: "explore",
    title: "Enterprise Vastu Audit",
    body: "Re-engineer physical workspaces to align with magnetic fields and natural elements — reducing friction, amplifying creative output.",
  },
  {
    icon: "insights",
    title: "Executive Transit Mapping",
    body: "Plan mergers, product launches, and hiring cycles around the most favorable planetary windows.",
  },
  {
    icon: "psychology",
    title: "Natal Leadership Profile",
    body: "Identify innate strengths and subconscious blockers in key decision-makers.",
  },
  {
    icon: "diversity_3",
    title: "Relationship Synastry",
    body: "Optimize co-founder dynamics and board composition through energetic compatibility.",
  },
  {
    icon: "hub",
          ...
          {services.map((s, i) => (
            ...
            >
              <div className="flex items-start justify-between">
                <Icon
                  name={s.icon}
                  className="text-[var(--gold)] text-[32px] transition-transform duration-500 group-hover:scale-110"
                />
...
function Council({ astrologers }: { astrologers: CouncilAstrologer[] }) {
  const ref = useReveal<HTMLDivElement>();
  ...
  return (
    <section
      id="council"
      className="py-32 border-t border-[var(--gold)]/15 bg-[var(--cream)]/40 scroll-mt-24"
    >
      <div ref={ref} className="reveal max-w-[1400px] mx-auto px-6 md:px-12">
        <div className="max-w-2xl mb-20">
          ...
          <h2 className="font-display text-5xl md:text-6xl text-foreground leading-[0.95]">
            ...
            <span className="italic text-[var(--gold)]">decide at scale.</span>
          </h2>
          <p className="text-muted-foreground mt-6 font-light leading-relaxed">
            Select an astrologer from our council to view their profile, services, and availability
            — then book your consultation.
          </p>
        </div>
              ...
              <div className="bg-card border border-[var(--gold)]/20 p-8">
                ...
                <ul className="space-y-4">
                  {stats.map((c) => (
                    <li
                      key={c.k}
                      className="flex items-baseline justify-between border-b border-[var(--gold)]/15 pb-3"
                    >
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background: "radial-gradient(circle at 30% 30%, rgba(200,151,61,0.30), transparent 60%)",
        }}
      />
  ...
  return (
    ...
    >
      {a.photo_url ? (
        ...
        <span className="font-display italic text-[var(--gold)] text-6xl">
          {councilInitials(a.full_name)}
        </span>
      )}
          ...
          <div className="col-span-12 lg:col-span-5">
            ...
            <ul className="space-y-5">
              {[
                ...
                <li
                  key={b}
                  className="flex items-baseline gap-5 border-b border-[var(--gold)]/15 pb-4"
                >
...
function ConsultFormFields() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiPost("/api/consultations/submit", form);
  ...
  const steps = [
    {
      n: "I",
      t: "Observe",
      body: "We map your natal chart, enterprise charter, and physical environment against current planetary positions.",
    },
    {
      n: "II",
      t: "Interpret",
      body: "Council members synthesize ancient texts with contemporary leadership data to surface latent leverage.",
    },
    {
      n: "III",
      t: "Implement",
