    ...
    <div className="min-h-screen bg-[var(--background)] text-foreground">
      ...
      <header className="relative pt-28 md:pt-32 pb-16 md:pb-24 overflow-hidden">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 80% 60% at 70% 20%, rgba(200,151,61,0.18), transparent 60%), radial-gradient(ellipse 50% 40% at 10% 80%, rgba(200,151,61,0.08), transparent 50%)",
          }}
        />
        <div className="relative max-w-[1400px] mx-auto px-6 md:px-12">
          <Link
            href="/#council"
            className="inline-flex items-center gap-2 text-xs label-caps text-muted-foreground hover:text-[var(--gold)] transition-colors mb-8"
          >
          ...
          <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
            <div className="lg:col-span-7 space-y-6 order-2 lg:order-1">
              {p.honorific && (
                <span className="label-caps text-[var(--gold)] tracking-[0.2em] text-xs">
                  {p.honorific}
                </span>
              )}
              ...
              {p.short_bio && !p.tagline && (
                <p className="text-muted-foreground leading-relaxed max-w-xl font-light">
                  {p.short_bio}
                </p>
              )}
            ...
            <div className="lg:col-span-5 order-1 lg:order-2">
              <div className="relative mx-auto max-w-[380px] lg:max-w-none">
                <div
                  className="absolute -inset-3 border border-[var(--gold)]/20 rounded-sm rotate-1"
                  aria-hidden
                />
                <div
                  className="absolute -inset-1 border border-[var(--gold)]/10 rounded-sm -rotate-1"
                  aria-hidden
                />
                {p.photo_url ? (
                  <img
                  ...
                  <div className="relative w-full aspect-[4/5] rounded-sm bg-[var(--charcoal)] border border-[var(--gold)]/30 flex items-center justify-center">
                    <span className="font-display text-8xl text-[var(--gold)]/30 italic">
          ...
          <main className="lg:col-span-8 space-y-20 md:space-y-28">
            ...
            {p.quote && (
              <blockquote className="relative py-10 px-8 md:px-12 border border-[var(--gold)]/20 bg-gradient-to-br from-[var(--cream)]/80 to-white rounded-sm">
                <Icon
                  name="format_quote"
                  className="absolute top-6 left-6 text-[var(--gold)]/25 text-5xl"
                />
            ...
            {services.length > 0 && (
              <section id="services">
                ...
                <p className="text-muted-foreground font-light mb-10 max-w-xl">
                  Select a session type that fits your needs. Each consultation is tailored to your
                  chart and context.
                </p>
            ...
            {availability.length > 0 && (
              <section id="availability">
                ...
                <p className="text-xs text-muted-foreground mt-4 font-light">
                  Hours shown for {DAYS_FULL[availability[0]?.day_of_week ?? 0]} timezone. Confirm
                  exact slots when booking.
                </p>
              </section>
            )}
          </main>
                ...
                <ul className="space-y-3 mb-6 text-sm">
                  {p.years_experience != null && (
                    <SidebarRow
                      icon="history"
                      label="Experience"
                      value={`${p.years_experience} years`}
                    />
                  )}
                  {languages.length > 0 && (
              ...
              {hasContact && (
                <div className="border border-[var(--gold)]/15 rounded-sm p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {p.email && (
                      <ContactChip href={`mailto:${p.email}`} icon="mail" label="Email" />
                    )}
                    ...
                    {p.website_url && (
                      <ContactChip href={p.website_url} icon="language" label="Web" external />
                    )}
                    {p.instagram_url && (
                      <ContactChip
                    ...
                    {p.linkedin_url && (
                      <ContactChip href={p.linkedin_url} icon="work" label="LinkedIn" external />
                    )}
                  </div>
                </div>
              )}
      ...
      <section className="bg-[var(--charcoal)] text-[var(--offwhite)] py-20 md:py-28 px-6 md:px-12">
        <div className="max-w-[900px] mx-auto text-center">
          <p className="label-caps text-[var(--gold-bright)] text-xs tracking-widest mb-4">
            Begin your consultation
          </p>
          <h2 className="font-display text-4xl md:text-5xl leading-tight mb-6">
            Work with {p.full_name.split(" ")[0]} on your{" "}
            <span className="italic text-[var(--gold-bright)]">cosmic roadmap</span>
          </h2>
          <p className="text-[var(--silver)]/80 font-light max-w-lg mx-auto mb-10 leading-relaxed">
            Share what you&apos;d like to explore. We&apos;ll open WhatsApp with your message ready
            to send to {p.full_name.split(" ")[0]}.
          </p>
...
function BookButton({
  ...
  const styles = {
    ...
    outline:
      "border border-[var(--gold)]/50 text-[var(--gold)] hover:bg-[var(--gold)] hover:text-[var(--background)]",
    "outline-light":
      "border border-[var(--gold-bright)] text-[var(--gold-bright)] hover:bg-[var(--gold-bright)] hover:text-[var(--charcoal)]",
  }[variant];
      ...
      <div className="min-w-0">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {(s.modes ?? []).map((m) => (
            <span
              key={m}
              className="inline-flex items-center gap-1.5 text-xs px-3 py-1 rounded-full border border-[var(--gold)]/30 text-[var(--gold)]"
            >
              <Icon
                name={m === "video" ? "videocam" : m === "phone" ? "call" : "person"}
                className="text-sm"
              />
              {MODE_LABELS[m] ?? m}
            </span>
