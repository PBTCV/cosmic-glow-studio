...
export function ConsultationsAdmin() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await apiPost<{ rows: Row[] }>(
  ...
  async function changeStatus(id: string, next: Status) {
    try {
      await apiPost("/api/consultations/status", { id, status: next }, token);
  ...
  async function submitNote() {
    ...
    setNoteBusy(true);
    try {
      await apiPost("/api/consultations/note", { id: noteTarget.id, note: noteText.trim() }, token);
    ...
    <>
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Review and manage client inquiries</p>
        </div>
              ...
              {!loading &&
                rows.map((r) => (
                  <TableRow key={r.id} className="cursor-pointer" onClick={() => setSelected(r)}>
                    ...
                    <TableCell
                      className="text-right align-top pt-4"
                      onClick={(e) => e.stopPropagation()}
                    >
