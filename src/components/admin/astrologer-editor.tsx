...
export function AstrologerEditor({
  id: idParam,
  initialTab,
}: {
  id: string;
  initialTab?: "profile" | "services" | "availability";
}) {
  ...
  const slugPreview =
    profile.slug?.trim() || slugify(profile.full_name) || "auto-generated-on-save";
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      }>("/api/astrologers/admin/get", { id: idParam }, token);
  ...
  async function saveProfile(andGoTo?: "services" | "availability") {
    ...
    try {
      ...
      if (isNew && res.id) {
        router.replace(
          andGoTo ? `/admin/astrologers/${res.id}?tab=${andGoTo}` : `/admin/astrologers/${res.id}`,
        );
      } else {
        void load();
  ...
  async function saveAllServices() {
    ...
    try {
      const saved = await Promise.all(
        services.map((s) =>
          apiPost<{ ok: boolean; id?: string }>("/api/astrologers/admin/service", s, token),
        ),
      );
  ...
  async function removeService(i: number) {
    ...
    if (s.id) {
      try {
        await apiPost("/api/astrologers/admin/service-delete", { id: s.id }, token);
      } catch (err) {
  ...
  const unsavedServices = useMemo(() => services.filter((s) => !s.id).length, [services]);
  ...
  async function saveAllAvail() {
    ...
    try {
      const saved = await Promise.all(
        availability.map((a) =>
          apiPost<{ ok: boolean; id?: string }>("/api/astrologers/admin/availability", a, token),
        ),
      );
  ...
  async function removeAvail(i: number) {
    ...
    if (a.id) {
      try {
        await apiPost("/api/astrologers/admin/availability-delete", { id: a.id }, token);
      } catch (err) {
        toast.error(err instanceof Error ? err.message : "Failed");
        return;
      }
    ...
    <div className="max-w-[1100px] mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
        <div>
          <Link
            ...
            className="text-xs text-muted-foreground hover:text-[var(--gold)]"
          >
            ← Back to astrologers
          </Link>
        ...
        <div className="flex flex-wrap gap-2">
          {profile.slug && profile.is_active && !isNew && (
            <Button variant="outline" size="sm" asChild>
              <Link href={`/astrologer/${profile.slug}`} target="_blank">
                Preview public page ↗
              </Link>
            </Button>
          )}
      ...
      {!profileReady && (
        <div className="mb-6 rounded-lg border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-900">
          Save the profile first to unlock <strong>Services</strong> and{" "}
          <strong>Availability</strong> tabs.
        </div>
      )}
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as "profile" | "services" | "availability")}>
        ...
        <TabsContent value="profile" className="space-y-8 mt-0">
          <EditorSection
            title="Identity"
            description="Basic info shown on the council section and profile page."
          >
            <FieldGrid>
              ...
              <FormField
                label="URL slug"
                error={errors.slug}
                hint={!errors.slug ? `Preview: /astrologer/${slugPreview}` : undefined}
              >
                <ValidatedInput
                  value={profile.slug ?? ""}
          ...
          <EditorSection title="Credentials">
            <FieldGrid>
              ...
              <FormField label="Lineage / Guru">
                <ValidatedInput
                  value={profile.lineage ?? ""}
                  onChange={(e) => updateField("lineage", e.target.value)}
                />
              </FormField>
          ...
          <div className="flex flex-wrap justify-end gap-2 sticky bottom-0 bg-[var(--background)]/95 backdrop-blur py-4 border-t border-[var(--gold)]/15">
            {!isNew && profileReady && (
              <Button
                variant="outline"
                onClick={() => void saveProfile("services")}
        ...
        <TabsContent value="services" className="space-y-4 mt-0">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Consultation offerings shown on the public profile.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {services.length > 0 && (
          ...
          {unsavedServices > 0 && (
            <p className="text-xs text-amber-700 bg-amber-500/10 border border-amber-500/20 rounded-md px-3 py-2">
              {unsavedServices} unsaved draft{unsavedServices === 1 ? "" : "s"} — click Save on each
              or use Save all.
            </p>
          )}
          ...
          {services.length === 0 ? (
            ...
            services.map((s, i) => (
              <div
                key={s.id ?? `draft-${i}`}
                className="border border-[var(--gold)]/15 rounded-xl p-5 space-y-4 bg-card/30"
              >
                ...
                <FieldGrid>
                  ...
                  <FormField label="Price">
                    <div className="flex gap-2">
                      <ValidatedInput
                        ...
                        onChange={(e) =>
                          updateService(i, {
                            price_amount: e.target.value === "" ? null : Number(e.target.value),
                          })
                        }
                        placeholder="Amount"
                      />
                      <Select
                        value={s.price_currency}
                        onValueChange={(v) => updateService(i, { price_currency: v })}
                      >
                  ...
                  <FormField label="Sort order">
                    <ValidatedInput
                      ...
                      onChange={(e) =>
                        updateService(i, { display_order: Number(e.target.value || 0) })
                      }
                    />
                  </FormField>
                  <div className="flex items-end pb-1">
                    <ToggleRow
                      label="Active on profile"
                      checked={s.is_active}
                      onCheckedChange={(v) => updateService(i, { is_active: v })}
                    />
                  </div>
                </FieldGrid>
        ...
        <TabsContent value="availability" className="space-y-4 mt-0">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Weekly hours shown on the public profile.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {availability.length > 0 && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={() => void saveAllAvail()}
                  disabled={savingAvail}
                >
                  {savingAvail ? "Saving all…" : "Save all"}
                </Button>
              )}
              <Button variant="outline" size="sm" onClick={addWeekdaySlots}>
                + Mon–Fri template
              </Button>
              <Button size="sm" onClick={() => addAvail()}>
                + Add slot
              </Button>
            </div>
          </div>
          ...
          {availability.length === 0 ? (
            <EmptyPanel
              ...
              action={
                <div className="flex gap-2 justify-center">
                  <Button variant="outline" onClick={addWeekdaySlots}>
                    + Mon–Fri template
                  </Button>
            ...
            availability.map((a, i) => (
              <div
                key={a.id ?? `slot-${i}`}
                className="border border-[var(--gold)]/15 rounded-xl p-5 bg-card/30"
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                  <FormField label="Day">
                    <Select
                      value={String(a.day_of_week)}
                      onValueChange={(v) => updateAvail(i, { day_of_week: Number(v) })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div className="flex gap-2 justify-end lg:justify-start">
                    <Button
                      ...
                      size="sm"
                      className="text-destructive"
                      onClick={() => void removeAvail(i)}
                    >
                      Remove
                    </Button>
                    <Button size="sm" onClick={() => void saveAvail(i)}>
                      Save
                    </Button>
                  </div>
                </div>
